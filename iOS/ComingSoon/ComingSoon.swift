...
struct ComingSoon: View {
  ...
  var body: some View {
    ...
    let scrollTrakingBinding = Binding {
      return scrollOffset
    } set: { newVal in
      scrollOffset = newVal
    ...
    // NavigationView {
      return Group {
        ZStack {
          ...
          TrackableScrollView(.vertical, showIndicators: false, contentOffset: scrollTrakingBinding) {
            LazyVStack {
              NotificationBar(showNotification: $showNotificationList)
              ForEach(Array(movies), id: \.offset) { index, movie in
                ComingSoonRow(movie: movie, movieDetailToShow: $movieDetailShow)
                  .frame(height: 400)
                  .overlay(
                    Group {
